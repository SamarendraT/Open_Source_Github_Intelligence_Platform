...
_EVENT_SCHEMA = (
    "event_id string, event_type string, create_at timestamp, event_date date, "
...
def test_daily_metrics_counts_by_type(spark):
    ...
    m = daily_repo_metrics(_events(aprk, rows)).collect()[0]
    assert m.stars == 2
    assert m.pushes == 2
    ...
    assert m.uniwue_human_actors == 7
...
def test_latest_repo_state_latest_name_and_language(spark):
    ...
    s = latest_repo_state(_events(aprk, rows)).collect()[0]
